...
# F. heteroclitus MDPP vs. MDPL: Brackish water acclimation treatment
...
```{r echo=TRUE}
res<-results(dds,contrast=c("condition","MDPL_BW","MDPP_BW"))
head(res)
...
res_ordered<-res_ordered[,c(1,26,2:7,8:10,17:19)]
write.csv(res_ordered, file="Fhet_MDPL_v_MDPP_BW_DESeq2_all.csv" )
```
...
write.csv(resSig,file="Fhet_MDPL_v_MDPP_BW_DESeq_padj0.05_log2FC1.csv")
# MA plot with gene names
plot(log2(res_ordered$baseMean), res_ordered$log2FoldChange, col=ifelse(res_ordered$padj < 0.05, "red","gray67"),main="MDPL_BW vs. MDPP_BW (padj<0.05, log2FC = ±1)",xlim=c(1,20),pch=20,cex=1,ylim=c(-12,12))
...
write.csv(up_down_FC,file="Fhet_DESeq_BW_MDPL_v_MDPP_padj0.01_updownlogFC2.csv")
dim(up_down_FC)
d<-up_down_FC
d<-na.omit(d)
dim(d)
...
heatmap.2(d, main="MDPL_BW vs. MDPP_BW (padj<0.01, log2FC = ±2)", 

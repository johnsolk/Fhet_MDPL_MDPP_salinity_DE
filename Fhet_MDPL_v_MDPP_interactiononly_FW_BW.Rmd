...
# F. heteroclitus MDPL vs. MDPP: experimental conditions are FW and BW
...
```{r echo=TRUE}
res<-results(dds, name="populationMDPL.conditionFW")
head(res)
...
write.csv(res_ordered, file="Fhet_interactiononly_BW_FW_DESeq2_all.csv" )
```
...
#write.csv(up_down_FC,file="Fhet_FW_BW_interactiononly_padj0.05_updownlogFC.csv")
dim(up_down_FC)
d<-up_down_FC
d<-na.omit(d)
dim(d)

---
title: "Fhet_MDPP_MDPL_DE_transfer"
author: "Lisa Cohen"
date: "7/15/2017"
...
# F. heteroclitus MDPP vs. MDPL: 24 hr fresh->brackish water transfer treatment
...
```{r echo=TRUE}
res<-results(dds,contrast=c("condition","MDPL_transfer","MDPP_transfer"))
head(res)
...
res_ordered<-res_ordered[,c(1,26,2:7,14:16,23:25)]
write.csv(res_ordered, file="Fhet_MDPL_v_MDPP_transfer_DESeq2_all.csv" )
```
...
write.csv(resSig,file="Fhet_MDPL_v_MDPP_transfer_DESeq_padj0.05_log2FC1.csv")
# MA plot with gene names
plot(log2(res_ordered$baseMean), res_ordered$log2FoldChange, col=ifelse(res_ordered$padj < 0.05, "red","gray67"),main="MDPL_transfer vs. MDPP_transfer (padj<0.05, log2FC = ±1)",xlim=c(1,20),pch=20,cex=1,ylim=c(-12,12))
...
write.csv(up_down_FC,file="Fhet_DESeq_transfer_MDPL_v_MDPP_padj0.05_updownlogFC1.csv")
dim(up_down_FC)
d<-up_down_FC
d<-na.omit(d)
dim(d)
...
heatmap.2(d, main="MDPL_transfer vs. MDPP_transfer (padj<0.05, log2FC = ±1)", 
